...
const QuizQuestions = (props) => {
	...
	const allQuizQuestions = useSelector((state) => state.quizQuestions.quizQuestions);
	const answerChecked = useSelector((state) => state.answerResponse.answerResponse);
	...
	useEffect(
		() => {
			if (pickedAnswer != null)
				dispatch(checkAnswerToQuestion(pickedAnswer[0], pickedAnswer[1], pickedAnswer[2]));
		},
		[ pickedAnswer ]
	);
